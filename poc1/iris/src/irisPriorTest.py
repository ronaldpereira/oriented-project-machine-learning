...
for specie in ['Iris-setosa', 'Iris-versicolor', 'Iris-virginica']:
    ...
    # pu_sl = pu_mr.PU_SL(prior=prior, basis='lm')

    # Using default prior (0.5)
    pu_sl = pu_mr.PU_SL(basis='lm')

    # Using real prior (0.3333)
    # pu_sl = pu_mr.PU_SL(prior=0.3333, basis='lm')
...
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.title('Receiver Operating Characteristic without Class-Prior Estimation')
plt.legend(loc="lower right")
plt.savefig('../output/pu_lm/graphics/pu_lm_without_cpe.pdf')
